...
public class HomeController {
    ...
    public String homePage(Model model) {
        ...
        if (true) {
            throw new RuntimeException("Something went wrong on the homepage!");
        }
        model.addAttribute("message", "Welcome to the Home Page!");
        return "home";  // This should map to home.jsp
    }

    // Handle specific error (in this case RuntimeException) on home page
    @ExceptionHandler(RuntimeException.class)
    public String handleRuntimeError(RuntimeException ex, Model model) {
        model.addAttribute("errorMessage", ex.getMessage());
        model.addAttribute("statusCode", 500);  // Internal Server Error
        return "error";  // This should map to error.jsp
    }
    
    // Handle generic errors in home controller
